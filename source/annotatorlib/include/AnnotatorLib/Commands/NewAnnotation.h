...
class ANNOTATORLIB_API NewAnnotation : public Command {
 public:
  ...
  NewAnnotation(unsigned long newObjectId, Class *newObjectClass, Frame *frame,
                float x, float y, float width, float height, Session *session,
                bool isFinished);

  NewAnnotation(Object *object, Frame *frame, float x, float y, float width,
                float height, Session *session, bool isFinished);
