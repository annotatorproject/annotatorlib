...
bool AnnotatorLib::Commands::CompressObject::execute() {
  removed_annotations = AnnotatorLib::Algo::CompressObjectTrack::compress(this->session, this->obj);
  return true;
}

bool AnnotatorLib::Commands::CompressObject::undo() {
  for( auto it = removed_annotations.begin(); it != removed_annotations.end(); it++) {
