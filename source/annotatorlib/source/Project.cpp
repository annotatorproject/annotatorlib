...
Project::Project(std::string name, ImageSetType imageSetType, std::string imageSetPath, StorageType storageType, std::string storagePath)
{
    this->name = name;
    this->imageSetType = imageSetType;
    this->storageType = storageType;
    this->storagePath = storagePath;
    this->imageSet = AnnotatorLib::ImageSetFactory::createImageSet(imageSetType, imageSetPath);
}
...
void Project::load() {
    ...
    {
        qDebug() << "File could not been opened: " << QString::fromStdString(this->path);
    ...
    this->imageSet = AnnotatorLib::ImageSetFactory::createImageSet(this->imageSetType, imageSetPath);

    loadStorage(root, this->storageType, this->storagePath);
    loadSession();
}

void Project::loadImageSet(QDomElement &root, ImageSetType &type, std::string &imageSetPath)
{
    QDomElement element = root.firstChildElement("ImageSet");
    type = AnnotatorLib::ImageSetTypeFromString(element.attribute("type").toStdString());
    imageSetPath = element.attribute("path").toStdString();
}

void Project::loadStorage(QDomElement &root, StorageType &type, std::string &storagePath)
{
    QDomElement element = root.firstChildElement("Storage");
    type = AnnotatorLib::StorageTypeFromString(element.attribute("type").toStdString());
    storagePath = element.attribute("path").toStdString();
}

void Project::loadRoot(QDomDocument &doc, QDomElement &root, std::string &name)
{
...
{
    AnnotatorLib::Storage::AbstractStorage *storage = AnnotatorLib::Storage::StorageFactory::createStorage(this->storageType);
...
QDomElement Project::saveImageSet(QDomDocument &doc)
{
    ...
    element.setAttribute("type", QString::fromStdString(AnnotatorLib::ImageSetTypeToString(this->imageSetType)));
    element.setAttribute("path", QString::fromStdString(this->imageSet->getPath()));
...
QDomElement Project::saveStorage(QDomDocument &doc)
{
    ...
    element.setAttribute("type", QString::fromStdString(AnnotatorLib::StorageTypeToString(this->storageType)));
...
{
    AnnotatorLib::Storage::AbstractStorage * storage = (AnnotatorLib::Storage::AbstractStorage *)session;
    storage->flush();
}

Project *Project::create(std::string name, ImageSetType imageSetType, std::string imageSetPath, StorageType storageType, std::string storagePath)
{
    if(name == "")
        throw std::runtime_error("Project name is empty");
    ...
    return  new Project(name, imageSetType, imageSetPath, storageType, storagePath);
}

Project *Project::create(std::string name, std::string imageSetType, std::string imageSetPath, std::string storageType, std::string storagePath)
{
    AnnotatorLib::ImageSetType iType = AnnotatorLib::ImageSetTypeFromString(imageSetType);
    AnnotatorLib::StorageType sType = AnnotatorLib::StorageTypeFromString(storageType);
...
bool Project::equals(Project *other)
{
    ...
    if(this->storageType != other->storageType)
        return false;
    if(this->storagePath != other->storagePath)
        return false;
    if(this->imageSetType != other->imageSetType)
        return false;
    if(!this->imageSet->equals(other->imageSet))
        return false;
    return true;
}

std::string Project::getImageSetPath()
{
