...
void SQLiteSaver::setPath(std::string path)
{
    this->path = path;
}

StorageType SQLiteSaver::getType()
{
    return AnnotatorLib::sqlite;
}
...
{
    char *zErrMsg = 0;
    std::string statement = "CREATE TABLE IF NOT EXISTS objects (id INTEGER, name TEXT);";
...
{
    char *zErrMsg = 0;
    std::string statement = "CREATE TABLE IF NOT EXISTS attributes (id INTEGER, name TEXT, type TEXT);";
    int rc = sqlite3_exec(db, statement.c_str(), 0, 0, &zErrMsg);
    sqlite3_free(zErrMsg);
    return rc;
}
...
{
    char *zErrMsg = 0;
    std::string statement = "CREATE TABLE IF NOT EXISTS annotations (id INTEGER, object INTEGER,"
