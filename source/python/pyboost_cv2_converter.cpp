...
#include "pyboostcvconverter.hpp"
    ...
    (!defined CV_VERSION_EPOCH && CV_VERSION_MAJOR == 2)
namespace pbcvt {
using namespace cv;
...
class NumpyAllocator : public MatAllocator {
 public:
  ...
  void allocate(int dims, const int* sizes, int type, int*& refcount,
    ...
    for (i = 0; i < dims - (cn > 1); i++) step[i] = (size_t)_strides[i];
...
matFromNDArrayBoostConverter::matFromNDArrayBoostConverter() {
  boost::python::converter::registry::push_back(
      ...
      matFromNDArrayBoostConverter::construct, boost::python::type_id<Mat>());
}
