...
TEST_F(algo_adjust_test, interpolate) {
    ...
    ASSERT_EQ(2.5, AnnotatorLib::Algo::AdjustAnnotation::interpolate(0, 10, 0.5, 1));
    ASSERT_EQ(2.5, AnnotatorLib::Algo::AdjustAnnotation::interpolate(0, 10, 1, 2));
...
TEST_F(algo_adjust_test, getInterpolation) {
  ...
  shared_ptr<Annotation> annotation = AnnotatorLib::Algo::AdjustAnnotation::getInterpolation(session, frame2, obj, 1);
