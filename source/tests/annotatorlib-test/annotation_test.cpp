...
TEST_F(annotation_test, genId) {
  ...
  shared_ptr<Annotation> annotation1 = Annotation::make_shared(
      make_shared<Frame>(1), obj, AnnotationType::RECTANGLE);
  shared_ptr<Annotation> annotation2 = Annotation::make_shared(
      make_shared<Frame>(2), obj, AnnotationType::RECTANGLE);
...
TEST_F(annotation_test, previousAndNext) {
  ...
  shared_ptr<Annotation> annotation5 = Annotation::make_shared(
      make_shared<Frame>(5), obj2, AnnotationType::RECTANGLE);
  shared_ptr<Annotation> annotation10 = Annotation::make_shared(
      make_shared<Frame>(10), obj2, AnnotationType::RECTANGLE);
  ...
  shared_ptr<Annotation> annotation15 = Annotation::make_shared(
      make_shared<Frame>(15), obj2, AnnotationType::RECTANGLE);
...
TEST_F(annotation_test, checkOrder) {
  ...
  unique_nmbs.erase(unique(unique_nmbs.begin(), unique_nmbs.end()),
                    unique_nmbs.end());
  unique_nmbs.shrink_to_fit();
  for (unsigned long nmb : unique_nmbs) {
    shared_ptr<Annotation> a =
        Annotation::make_shared(make_shared<Frame>(nmb), obj);
...
TEST_F(annotation_test, annotationLifeTime) {
  ...
  if (true) {
    shared_ptr<Annotation> annotation2 =
        Annotation::make_shared(frame2, obj, AnnotationType::RECTANGLE);
    ASSERT_EQ(obj->getAnnotations().size(), (unsigned long)2);
    ASSERT_EQ(frame1->getAnnotations().size(), (unsigned long)1);
    ASSERT_EQ(frame2->getAnnotations().size(), (unsigned long)1);
    annotation2.reset();
  }
  ASSERT_EQ(obj->getAnnotations().size(),
            (unsigned long)1);  // was annotation2 automatically unregistered?
  ASSERT_EQ(frame1->getAnnotations().size(), (unsigned long)1);
  ASSERT_EQ(frame2->getAnnotations().size(),
            (unsigned long)0);  // was annotation2 automatically unregistered?
}
